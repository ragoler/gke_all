// ==============================================================================
// GKE Agent Sandbox (Kata / MicroVM) - Playroom Controller (Static / features)
// ==============================================================================
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchClaims() {
        try {
            const response = await fetchWithAuth("/api/features/sandbox-kata/sandboxes");
        ...
        claims.forEach(item => {
            ...
            card.innerHTML = `
                <div class="sandbox-item-header">
                    ...
                    <span class="sandbox-state">RUNNING (Kata VM)</span>
                </div>
    ...
    window.deleteClaim = async (id) => {
        ...
        try {
            const response = await fetchWithAuth(`/api/features/sandbox-kata/sandboxes/${id}`, {
                method: "DELETE"
            });
    ...
    window.sendPrompt = async (id) => {
        ...
        const { div: aiDiv, msgObj: aiMsg } = appendChatMessage(id, "Executing inside Kata MicroVM...", "ai");
        const provider = document.getElementById(`provider-${id}`).value;
        
        try {
            const response = await fetchWithAuth(`/api/features/sandbox-kata/sandboxes/${id}/message`, {
                method: "POST",
    ...
    window.requestQuote = async (id) => {
        ...
        try {
            const response = await fetchWithAuth(`/api/features/sandbox-kata/sandboxes/${id}/quote`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
