"""Hub integration router for the Agent Sandbox feature.

The Hub auto-discovers this module via ``hub_router`` in feature.yaml and mounts
``router`` under ``/api/features/agent-sandbox`` (see showcase_admin/app/features.py).
...
FEATURE_NAME = "agent-sandbox"
VLLM_FEATURE_NAME = "gpu-inference"


@router.get("/sandboxes", summary="List active gVisor sandbox claims")
